...
public class ExpressionTry implements Expression {
	Expression body, elsepart;
	
	@Override
	public Object eval(Scope scope) {
		try {
			return body.eval(body instanceof ExpressionMulti
					? new Scope(scope) : scope);
		} catch (CalculatorError | IndexOutOfBoundsException e) {
			Console.lastError = e;
			return elsepart.eval(elsepart instanceof ExpressionMulti
					? new Scope(scope) : scope);
		}
	}
	...
	public Object evalOptionalValue(Scope scope) {
		try {
			return body.evalOptionalValue(
					body instanceof ExpressionMulti? new Scope(scope)
							: scope);
		} catch (CalculatorError | IndexOutOfBoundsException e) {
			Console.lastError = e;
	...
	public String toCompiledString() {
		return "<TRY %s ELSE %s>".format(
				(Object) body.toCompiledString(),
