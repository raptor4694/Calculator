...
public class ExpressionXAssignOp implements ExpressionNamed {
	...
	public Object eval(Scope scope) {
		...
		return array[0] = toNumber(
				operator.call(scope, array[0], value.eval(scope)));
	}
	...
	public String toCompiledString() {
		...
		case ADD:
		case SUBTRACT:
			return "<%s %s %s <.X %s>>".format(operator,
					value.toCompiledString(), operator.getVerb(),
					array.toCompiledString());
		default:
			return "<%s <.X %s> %s %s>".format(operator,
	...
	public String toString() {
		return "XAssignOp{array=%s,operator=%s,value=%s}".format(array,
				operator, value);
	}
