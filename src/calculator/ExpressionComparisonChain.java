...
public class ExpressionComparisonChain implements Expression {
	...
	public static class Op {
		...
		static {
			...
			for (EnumOperator operator : operators) {
				VALUES[operator.ordinal()][0] =
						new Op(operator, false);
				VALUES[operator.ordinal()][1] = new Op(operator, true);
			}
		}
		
		public static Op of(EnumOperator operator,
				boolean elementWise) {
	...
	public Object eval(Scope scope) {
		...
		for (int opIdx = 0; opIdx < operators.size(); opIdx++) {
			...
			Object result =
					op.operator.call(op.elementWise, last, current);
			if (!toBoolean(result))
				return 0.0;
			last = current;
		}
	...
	public String toEvalString() {
		...
		for (int i = 0; i < operators.size(); i++) {
			...
			if (op.elementWise)
				b.append('.');
			b.append(op.operator.getSymbol()).append(' ').append(
					ex.toEvalString());
		}
		return b.toString();
	}
	...
	public String toCompiledString() {
		...
		for (int i = 0; i < operators.size(); i++) {
			...
			if (op.elementWise)
				b.append('.');
			b.append(op.operator).append(' ').append(
					ex.toCompiledString());
		}
		b.append('>');
		return b.toString();
	...
	@Override
	public String toString() {
		return "ComparisonChain{exprs=%s,operators=%s}".format(exprs,
				operators);
	}
