...
public class ExpressionMulti implements Expression {
	...
	public Object eval(Scope scope) {
		...
		for (int i = 0; i < exprs.length; i++) {
			...
			scope.setVariable("ans", last = i == exprs.length - 1? expr.eval(scope)
	...
	public Object evalOptionalValue(Scope scope) {
		...
		for (Expression expr : exprs)
			scope.setVariable("ans", last = expr.evalOptionalValue(scope));
		return last;
	}
	...
	public String toString() {
		return "Multi{exprs=%s}".format((Object) Arrays.toString(exprs));
	}
