...
public class ExpressionFor implements Expression {
	...
	public final Expression start, end, increment, body;
	...
	public String toCompiledString() {
		if (increment == null)
			return "<FOR \"%s\" FROM %s TO %s DO %s>".format((Object) variable,
					start.toCompiledString(), end.toCompiledString(),
					body.toCompiledString());
		else
			return "<FOR \"%s\" FROM %s TO %s INCREMENT %s DO %s>".format(
					(Object) variable, start.toCompiledString(),
					end.toCompiledString(), increment.toCompiledString(),
					body.toCompiledString());
	}
